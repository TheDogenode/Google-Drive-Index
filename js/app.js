...
function nav(path) {
  ...
  <a class="navbar-brand" href="//index.hashhackers.com">${UI.logo_image ? '<img border="0" alt="'+UI.company_name+'" src="'+UI.logo_link_name+'" height="'+UI.height+'" width="'+UI.logo_width+'">' : UI.logo_link_name}</a>
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
      <li class="nav-item">
        <a class="nav-link" href="//index.hashhackers.com">Home</a>
      </li>`;
    var names = window.drive_names;
    var drive_name = window.drive_names[cur];

    html += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Current Path</a><div class="dropdown-menu" aria-labelledby="navbarDropdown"><a class="dropdown-item"  href="//index.hashhackers.com">> Home</a>`;
...
<form class="d-flex" method="get" action="//index.hashhackers.com/0:search">
    ...
    <div class="container">
       ...
       <div class="row justify-content-center">
          <div class="col-12 col-md-10 col-lg-8">
             <form class="card card-sm" method="get" action="//index.hashhackers.com/0:search">
                <div class="card-body row no-gutters align-items-center">
                      <input class="form-control form-control-lg form-control-borderless" type="search" placeholder="Search Bhadoo Drives" name="q"  aria-label="Search" required>
                </div>
...
function append_search_result_to_list(files) {
    ...
    for (i in files) {
        ...
        item['size'] = formatFileSize(item['size']);
        if (item['mimeType'] == 'application/vnd.google-apps.folder') {
            html += `<a style="color: ${UI.folder_text_color};" onclick="onSearchResultItemClick(this)" data-bs-toggle="modal" data-bs-target="#SearchModel" id="${item['id']}" teamDriveId=${item['teamDriveId']} class="list-group-item list-group-item-action"><svg width="1.5em" height="1.5em" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48"><linearGradient id="WQEfvoQAcpQgQgyjQQ4Hqa" x1="24" x2="24" y1="6.708" y2="14.977" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#eba600"></stop><stop offset="1" stop-color="#c28200"></stop></linearGradient><path fill="url(#WQEfvoQAcpQgQgyjQQ4Hqa)" d="M24.414,10.414l-2.536-2.536C21.316,7.316,20.553,7,19.757,7L5,7C3.895,7,3,7.895,3,9l0,30	c0,1.105,0.895,2,2,2l38,0c1.105,0,2-0.895,2-2V13c0-1.105-0.895-2-2-2l-17.172,0C25.298,11,24.789,10.789,24.414,10.414z"></path><linearGradient id="WQEfvoQAcpQgQgyjQQ4Hqb" x1="24" x2="24" y1="10.854" y2="40.983" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#ffd869"></stop><stop offset="1" stop-color="#fec52b"></stop></linearGradient><path fill="url(#WQEfvoQAcpQgQgyjQQ4Hqb)" d="M21.586,14.414l3.268-3.268C24.947,11.053,25.074,11,25.207,11H43c1.105,0,2,0.895,2,2v26	c0,1.105-0.895,2-2,2H5c-1.105,0-2-0.895-2-2V15.5C3,15.224,3.224,15,3.5,15h16.672C20.702,15,21.211,14.789,21.586,14.414z"></path></svg> ${item.name} ${UI.display_time ? `<span class="badge bg-info float-sm-end"> ` + item['modifiedTime'] + ` </span>` : ``}</a>`;
        } else {
            var p = '/' + cur + ':/' + item.name;
            var c = "file";
            ...
            html += `<a style="color: ${UI.css_a_tag_color};" onclick="onSearchResultItemClick(this)" data-bs-toggle="modal" data-bs-target="#SearchModel" id="${item['id']}" teamDriveId=${item['teamDriveId']} gd-type="${item.mimeType}" class="list-group-item list-group-item-action">`
...
function onSearchResultItemClick(a_ele) {
    ...
    if (teamDriveId == '0AL6Tka2S7BPKUk9PVA'){
      var searchhost = 'https://s1.hashhackers.com/0'; // drive 1
    }
    else if (teamDriveId == '0ABqcOIJGMM79Uk9PVA'){
      var searchhost = 'https://s1.hashhackers.com/1'; // drive 2
    }
    else if (teamDriveId == '0AEZqQMn_xaNRUk9PVA'){
      var searchhost = 'https://s1.hashhackers.com/2'; // drive 3
    }
    else if (teamDriveId == '0AOM2i7MQiuWIUk9PVA'){
      var searchhost = 'https://s1.hashhackers.com/3'; // drive 4
    }
    else if (teamDriveId == '0APPJFuLuTq7jUk9PVA'){
      var searchhost = 'https://s1.hashhackers.com/4'; // drive 5
    }
    else if (teamDriveId == '0AEarpkDuiz9gUk9PVA'){
      var searchhost = 'https://s2.hashhackers.com/0'; // drive 6
    }
    else if (teamDriveId == '0AB0U5rFDybQFUk9PVA'){
      var searchhost = 'https://s2.hashhackers.com/1'; // drive 7
    }
    else if (teamDriveId == '0AIZwiPqjaAqFUk9PVA'){
      var searchhost = 'https://s2.hashhackers.com/2'; // drive 8
    }
    else if (teamDriveId == '0AMQP-8gNxWfaUk9PVA'){
      var searchhost = 'https://s2.hashhackers.com/3'; // drive 9
    }
    else if (teamDriveId == '0AIFitXHtO03HUk9PVA'){
      var searchhost = 'https://s2.hashhackers.com/4'; // drive 10
    }
    else if (teamDriveId == '0APtl5rsd5FHQUk9PVA'){
      var searchhost = 'https://s3.hashhackers.com/0'; // drive 11
    }
    else if (teamDriveId == '0AF1aihIoC2JXUk9PVA'){
      var searchhost = 'https://s3.hashhackers.com/1'; // drive 12
    }
    else if (teamDriveId == '0AInmBU1dHhwJUk9PVA'){
      var searchhost = 'https://s3.hashhackers.com/2'; // drive contributors
    }
    else if (teamDriveId == '0AFmP8fiI9UEsUk9PVA'){
      var searchhost = 'https://s3.hashhackers.com/3'; // drive mega
    }
    else if (teamDriveId == '0ADolfPBpwPHWUk9PVA'){
      var searchhost = 'https://s3.hashhackers.com/4'; // drive big
    }
    else if (teamDriveId == '0AL0LGwbBkeTuUk9PVA'){
      var searchhost = 'https://s4.hashhackers.com/0'; // drive proper organizing
    }
    else if (teamDriveId == '0AMYjRTXTpjhIUk9PVA'){
      var searchhost = 'https://s4.hashhackers.com/1'; // drive bhadoo torrent group
    }
    else if (teamDriveId == '0AMwQ3VGeqNvPUk9PVA'){
      var searchhost = 'https://s4.hashhackers.com/2'; // study fever tar
    }
    else {
      var searchhost = 'https://index.hashhackers.com/0';
    }
